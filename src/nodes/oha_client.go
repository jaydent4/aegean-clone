...
func NewOHAClient(name, host string, port int, next []string, readyNodes []string, runConfig map[string]any, requestLogic func(c *OHAClient)) *OHAClient {
	if requestLogic == nil {
...
func (c *OHAClient) WaitForRequestCompletion(requestID any) {
...
func (c *OHAClient) WaitForNodesReady(nodeNames []string) {
	for {
		allReady := true
