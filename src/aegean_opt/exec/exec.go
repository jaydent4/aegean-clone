...
const (
	...
	ingressScheduledRequestEvent
	ingressVerificationWindowClosedEvent
	ingressStreamExecutionCompleteEvent
	ingressNestedReadyEvent
)
...
type Exec struct {
	...
	streamWindows         map[int]*streamWindowRuntime
	streamActiveSeq       int
	streamWorkerChs       []chan streamWorkerTask
...
func NewExec(name string, verifiers []string, peers []string, verifierCh chan<- map[string]any, shimCh chan<- map[string]any, verifyResponseQuorumSize int, executeRequest ExecuteRequestFunc, initStateFn InitStateFunc, runConfig map[string]any) *Exec {
	...
	exec := &Exec{
		...
		streamWindows:         make(map[int]*streamWindowRuntime),
	...
	exec.streamWorkerChs = make([]chan streamWorkerTask, exec.workerCount)
...
func (e *Exec) BufferNestedResponse(payload map[string]any) bool {
	...
	ok = e.scheduler.enqueueNestedResponse(requestID, payload)
	if ok {
		select {
		case e.ingressCh <- ingressEvent{kind: ingressNestedReadyEvent}:
		default:
		}
	}
	return ok
...
func (e *Exec) HandleScheduledRequestMessage(payload map[string]any) map[string]any {
	seqNum := common.GetInt(payload, protocol.FieldSeqNum)
	e.ingressCh <- ingressEvent{kind: ingressScheduledRequestEvent, payload: payload}
	return map[string]any{"status": "buffered", "seq_num": seqNum}
}

func (e *Exec) HandleVerificationWindowClosedMessage(payload map[string]any) map[string]any {
	seqNum := common.GetInt(payload, protocol.FieldSeqNum)
	e.ingressCh <- ingressEvent{kind: ingressVerificationWindowClosedEvent, payload: payload}
	return map[string]any{"status": "buffered", "seq_num": seqNum}
}
...
func (e *Exec) requestPayloadsForSeq(seqNum int) []map[string]any {
	...
	if requests, ok := batchPayload[protocol.FieldRequests].([]map[string]any); ok {
		return requests
	}
	if rawRequests, ok := batchPayload[protocol.FieldRequests].([]any); ok {
...
func (e *Exec) applyIngressEvent(ev ingressEvent) {
	...
	case ingressScheduledRequestEvent:
		e.applyScheduledRequestEvent(ev.payload)
	case ingressVerificationWindowClosedEvent:
		e.applyVerificationWindowClosedEvent(ev.payload)
	case ingressStreamExecutionCompleteEvent:
		e.applyStreamExecutionResult(ev.streamResult)
	case ingressNestedReadyEvent:
		// Nested responses are stored in execScheduler. The drain pass will
		// deterministically wake any stream request whose response count changed.
	default:
		return
	}
}
...
func (e *Exec) drainBufferedMessages() bool {
	...
	return bResp || sResp || vResp || vrResp
}
