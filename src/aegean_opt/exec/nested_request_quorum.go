...
const (
	// TODO: make the nested EO request quorum configurable once opt configs settle.
...
func (q *NestedEORequestQuorumGate) HandleNestedRequestMessage(payload map[string]any) map[string]any {
	...
	requestID, ok := canonicalRequestID(payload[protocol.FieldRequestID])
	if !ok {
	...
	return q.recordVote(source, requestID, stringSliceFromAny(payload[protocol.FieldTargets]), request)
}
...
func (q *NestedEORequestQuorumGate) forwardMessage(source string, requestID string, targets []string, payload map[string]any) map[string]any {
	return map[string]any{
		protocol.FieldType:      protocol.MessageTypeEONestedRequest,
		protocol.FieldRequestID: requestID,
		protocol.FieldRequest:   cloneMapAny(payload),
		protocol.FieldTargets:   append([]string{}, targets...),
		protocol.FieldHash:      nestedEORequestHash(requestID, payload),
		"sender":                source,
	}
}
